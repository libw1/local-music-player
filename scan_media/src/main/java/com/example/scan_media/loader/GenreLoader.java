...
public class GenreLoader {
    ...
    public static Observable<ArrayList<Genre>> getAllGenres(@NonNull final Context context) {
        return Observable.create(new Observable.OnSubscribe<ArrayList<Genre>>() {
            @Override
            public void call(Subscriber<? super ArrayList<Genre>> subscriber) {
                subscriber.onNext(getGenresFromCursor(context, makeGenreCursor(context)));
            }
        });
    }
